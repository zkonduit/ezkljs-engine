  ...
  it('field serialization round trip fuzzing', async () => {
    ...
    for (let i = 0; i < numRuns; i++) {
      ...
      for (let j = 0; j <= maxScale; j++) {
        ...
        let integerOutput = uint8ArrayToJsonObject(
          new Uint8Array(result3.buffer),
        )
        if (i == 0 && j == 0)
        ...
        if (i == 0 && j == 0)
          console.debug('Vec u64s to field element output (hex)', feltHexOutput)
        let referenceFelt = intToFieldElement(integerOutput)
        if (i == 0 && j == 0)
          console.debug('Reference field element (decimal)', referenceFelt)
